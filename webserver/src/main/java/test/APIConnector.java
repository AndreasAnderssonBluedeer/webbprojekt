...
public class APIConnector {
	 ...
	 * @return String
	 */
	public Movie getInfo(String query){
	 ...
	 * @return String
	 */
	public Trailer getTrailer(String query) {
		...
		try {
			...
			if (status.getStatusCode() == 200) {
				...
				try {
					...
					// Something didn't went well.
					System.out.println(
							"************TrailerAddict API didn't respond or couldn't find the trailer.1 ************");
				}
			} else {
				// Something didn't went well.
				System.out.println(
						"************TrailerAddict API didn't respond or couldn't find the trailer. 2************");
			}
		} catch (IOException e) {
	...
	public String getTrailerTitle(String imdbID) {
		
		Trailer t = new Trailer();
		try {
		
			httpclient = HttpClients.createDefault();
			httpGet = new HttpGet("http://api.traileraddict.com/?imdb="+imdbID+"&count=1&width=680");
